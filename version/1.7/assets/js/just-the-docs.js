...
function initSearch() {
  ...
  request.open('GET', '/version/1.7/assets/js/search-data.json', true);
...
jtd.setTheme = function(theme) {
  ...
  cssFile.setAttribute('href', '/version/1.7/assets/css/just-the-docs-' + theme + '.css');
}
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('pre code.language-xml').forEach(block => {
        block.innerHTML = block.innerHTML.replace(/&lt;\/?(\w+)([^&gt;]*?)&gt;/g, (match, p1, p2) =>
            `<a href="/version/1.7/xml/${p1}" class="xml-tag-link">${match}</a>`);
    });
});
