...
- The `<rows>max</rows>` tag fetches the maximum number of rows (currently 10.000)
...
- ui: message when search with no matches
- keycloak: token support

## 1.7.1

- db: upgrade could stop on `db:migrate`
- wsl: new image with fixed environment variables
...
- Sorting of nordic characters for users and archives. Will be solved as part of new database platform in another release.
- Switching language can jump to another page
- For local installs: .env file: Add CAS_EXTERNAL_URL=http://server-dns-name:8181
