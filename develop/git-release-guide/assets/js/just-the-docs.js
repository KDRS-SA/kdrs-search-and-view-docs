...
function initSearch() {
  ...
  request.open('GET', '/develop/git-release-guide/assets/js/search-data.json', true);
...
jtd.setTheme = function(theme) {
  ...
  cssFile.setAttribute('href', '/develop/git-release-guide/assets/css/just-the-docs-' + theme + '.css');
}
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('pre code.language-xml').forEach(block => {
        block.innerHTML = block.innerHTML.replace(/&lt;\/?(\w+)([^&gt;]*?)&gt;/g, (match, p1, p2) =>
            `<a href="/develop/git-release-guide/xml/${p1}" class="xml-tag-link">${match}</a>`);
    });
});
